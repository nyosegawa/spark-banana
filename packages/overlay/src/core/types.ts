...
/** SparkAnnotation コンポーネントの設定 */
export interface SparkAnnotationConfig {
  /** Bridge ServerのWebSocket URL (default: ws://localhost:3700) */
  bridgeUrl?: string;
  /** プロジェクトルート。指定時は最優先。未指定時は VITE_SPARK_PROJECT_ROOT / NEXT_PUBLIC_SPARK_PROJECT_ROOT を参照 */
  projectRoot?: string;
  /** ツールバーの初期位置 */
